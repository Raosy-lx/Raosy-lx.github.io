...
CMemPool* CMemPool::m_MemPoolInstance = NULL;
    ...
    std::mutex CMemPool::cout_mutex;
    std::mutex CMemPool::buddy_mutex;
#endif
...
struct kmem_cache_s {
    ...
    u_int32_t           objectsInSlab;
    u_int64_t           num_active;
    u_int64_t           num_allocations;
    std::mutex          cache_mutex;
    u_int32_t           order;
    u_int32_t           colour_max;
    u_int32_t           colour_next;
    bool                growing;
    void (*ctor)(void *);
    void (*dtor)(void *);
    int                 error_code;
...
kmem_cache_t CMemPool::cache_cache;
kmem_cache_t* CMemPool::allCaches = NULL;

void CMemPool::kmem_cache_allInfo()
{
    kmem_cache_t* curr = CMemPool::allCaches;
    while (curr != nullptr)
    {
...
CMemPool* CMemPool::GetInstance() {
    ...
    // // after C++11
    // static CMemPool s_MempoolObject;
    // return &s_MempoolObject;
#else
    m_MemPoolInstance = new CMemPool();
    return m_MemPoolInstance;
#endif
...
void CMemPool::kmem_init(void* space, int block_num)
{
    ...
    slab->next = nullptr;
    slab->prev = nullptr;
    slab->m_Cache = &cache_cache;

    long memory = (1 << cache_cache.order)*BLOCK_SIZE;
    memory -= sizeof(slab_t);
    int n = 0;
    while ( (long)(memory - sizeof(u_int32_t) - cache_cache.objectSize) >= 0 )
    {
        n++;
        memory -= sizeof(u_int32_t) + cache_cache.objectSize;
    }

    slab->objects = (void*)((char*)ptr + sizeof(slab_t) + sizeof(u_int32_t)*n);
...
void CMemPool::kmem_cache_info(kmem_cache_t *cachep) // Print cache info
{
...
int CMemPool::kmem_cache_error(kmem_cache_t *cachep) // Print error message
{
    lock_guard<mutex> guard1(cout_mutex);
...
kmem_cache_t* CMemPool::kmem_cache_create(const char *name, size_t size, void(*ctor)(void *), void(*dtor)(void *)) // Allocate cache
{
...
int CMemPool::kmem_cache_shrink(kmem_cache_t *cachep) // Shrink cache
{
	if (cachep == nullptr)
		return 0;
...
void* CMemPool::kmem_cache_alloc(kmem_cache_t *cachep) // Allocate one object from cache
{
...
void CMemPool::kmem_cache_free(kmem_cache_t *cachep, void *objp) // Deallocate one object from cache
{
...
void* CMemPool::kmalloc(size_t size) // Alloacate one small memory buffer
{
...
kmem_cache_t* CMemPool::find_buffers_cache(const void* objp)
{
...
void CMemPool::kfree(const void *objp) // Deallocate one small memory buffer
{
	if (objp == nullptr)
		return;
...
void CMemPool::kmem_cache_destroy(kmem_cache_t *cachep) // Deallocate cache
{
	if (cachep == nullptr || *cachep->name == '\0')
		return;
